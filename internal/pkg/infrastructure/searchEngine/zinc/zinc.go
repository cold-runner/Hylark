...
type zincClient struct {
	...
	hzClient *client.Client
}

func NewSearchEngine(opt *instance.ZincClientConfig) searchEngine.SearchEngine {
...
func (c *zincClient) do(ctx context.Context, method string, body []byte, url string) (*protocol.Response, error) {
	...
	req.SetRequestURI(pre + c.Host + url)
	req.SetBasicAuth(c.User, c.Password)
	req.SetMethod(method)
	req.SetBody(body)

	if err := c.hzClient.Do(ctx, req, resp); err != nil {
		return nil, err
	}
